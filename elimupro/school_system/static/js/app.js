...
const CBC_COLORS = { E: "#16a34a", M: "#0ea5e9", A: "#f59e0b", B: "#ef4444" };
...
const scaleLegendHtml = () => `<div class="chart-legend" style="margin-top:10px">
    ${Object.keys(CBC_LEVELS).map(k => `<span><span class="dot" style="background:${CBC_COLORS[k]}"></span><b>${k}</b> — ${CBC_LEVELS[k].name} (${CBC_LEVELS[k].band})</span>`).join("")}
  </div>`;
...
document.addEventListener("keydown", (e) => {
  if (e.key === "Escape") {
    // close the Ctrl+K palette first if it is open (the UI promises "Esc close")
    const pal = document.getElementById("palette");
    if (pal && !pal.classList.contains("hidden")) { pal.classList.add("hidden"); return; }
    closeModal();
  }
});
...
/* enterApp — show the app shell after a successful sign-in (inverse of showLogin).
   This function was referenced by doLogin() and boot() but had been dropped from
   the file, which froze the app on the login screen after authentication. */
function enterApp() {
  document.getElementById("login-screen").classList.add("hidden");
  document.getElementById("app").classList.remove("hidden");
  // reset per-user cached state so one account's data never shows for another
  state.gchildren = null;
  state.guardianChild = null;
  state.students = null;
  state.classes = null;
  ...
  if (av) av.innerHTML = u.profile_pic ? `<img src="${esc(u.profile_pic)}" alt="">` : esc(initials(u.name || u.full_name || u.username || "?"));
  const nm = $("#user-name");
  if (nm) nm.textContent = u.name || u.full_name || u.username || "User";
  const rl = $("#user-role");
  if (rl) rl.textContent = ({ admin: "Administrator", teacher: "Teacher", accounts: "Accounts", librarian: "Librarian", guardian: "Parent", superadmin: "Platform admin" })[u.role] || u.role || "";
  // school identity in the sidebar + term badge
  const s = state.settings || {};
  const sb = $("#sidebar-school");
  if (sb) sb.textContent = s.school_name || "ElimuPro";
  const tb = $("#term-badge");
  if (tb) tb.textContent = (s.current_term || "") + (s.academic_year ? " · " + s.academic_year : "");
  const vt = $("#version-tag");
  if (vt) vt.textContent = "ElimuPro " + (window.__epVersion || "");
  applyAppearance();
  renderNav();
  OFFLINE.pending = loadQueue().length;
  updateNetUI();
  // landing view depends on the role
  const first = u.role === "guardian" ? "gdash"
    : u.role === "librarian" ? "library"
    : u.role === "superadmin" ? "schools"
    : "dashboard";
  state.view = first;
  openView(first);
}
...
$("#logout-btn").addEventListener("click", async () => {
  ...
  state.user = null;
  // clear cached per-user data so the next sign-in starts clean
  state.gchildren = null;
  state.guardianChild = null;
  state.students = null;
  state.classes = null;
...
async function myAccount() {
  ...
  <div class="modal-body">
    ...
    <div class="form-grid">
      ...
      <div><label>Role</label><input class="input" value="${esc({admin:"Administrator",teacher:"Teacher",accounts:"Accounts",librarian:"Librarian",guardian:"Parent / Guardian",superadmin:"Platform Admin"}[u.role] || u.role)}" disabled></div>
...
async function view_dashboard(el) {
  ...
  const gItems = Object.keys(dist).map(g => ({ label: g, value: dist[g], color: CBC_COLORS[g] || "#64748b" }));
  ...
  <div class="modal-body">
    ...
    ${latest ? `
      <div class="kgrid" style="grid-template-columns:repeat(4,1fr)">
        ...
        <div class="kpi"><div class="k">${dScale === "cbc" ? "Level" : "Mean grade"}</div><div class="v"><span class="grade-pill ${gradeClass(mg)}">${mg || "—"}</span></div></div>
...
/* per-term conduct rating from net merits (CBC holistic development) */
function conductRating(merits, demerits) {
  const net = (merits || 0) - (demerits || 0);
  if (net >= 5) return "Excellent";
  if (net >= 1) return "Good";
  if (net >= -2) return "Satisfactory";
  ...
  const showTop = () => {
    ...
    <div class="card"><div class="card-head"><h3>Students with the best conduct</h3><p>Net merits this term</p></div>
      <div class="table-wrap"><table class="tbl">
        ...
        <tbody>${sum.top.map((t, i) => {
          const rating = conductRating(t.merits, t.demerits);
          return `<tr>
          ...
          <td><span class="badge ${CONDUCT_STYLES[rating] || "b-slate"}">${esc(rating)}</span></td>
...
async function view_gfees(el) {
  ...
  const gpayBtn = $("#gp-pay");
  if (gpayBtn) gpayBtn.addEventListener("click", async () => {
