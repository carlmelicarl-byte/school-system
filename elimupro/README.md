...
```bash
cd school_system
pip install -r requirements.txt   # flask (dev server) + openpyxl (Excel export) + waitress/gunicorn (production)
...
└── static/
    ...
    ├── js/app.js     # SPA: routing, 25+ views, SVG charts (no CDN)
    ├── sw.js         # service worker — offline app shell + API cache (served at /sw.js)
...
Legacy deployments (pre-production-pack) used a demo secret key and plain SHA-256 password hashes. Current builds hash passwords with salted **scrypt** (legacy hashes auto-upgrade on login), rate-limit logins, and take the secret key from `ELIMUPRO_SECRET` — see the production pack above and **[DEPLOY.md](DEPLOY.md)** for everything to harden before a public deployment.
